...
abstract class SuperBottomSheetFragment : BottomSheetDialogFragment() {
        ...
        with(requireContext().getAttrId(R.attr.superBottomSheet_statusBarColor)) {
            when (this) {
                INVALID_RESOURCE_ID -> ContextCompat.getColor(
                    requireContext(),
                    requireContext().getAttrId(android.R.attr.colorPrimaryDark)
                )
                else -> ContextCompat.getColor(requireContext(), this)
            }
        }
    ...
    open fun isSheetAlwaysExpanded() =
    ...
    open fun isSheetCancelableOnTouchOutside() =
    ...
    open fun getExpandedHeight() =
        with(requireContext().getAttrId(R.attr.superBottomSheet_expandedHeight)) {
            when (this) {
