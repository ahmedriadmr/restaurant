...
class RubberRangePicker : View {
        ...
        attrs?.let {
            ...
            if (typedArray.hasValue(R.styleable.RubberRangePicker_initialStartValue)) {
                setCurrentStartValue(
                    typedArray.getInt(
                        R.styleable.RubberRangePicker_initialStartValue,
                        minValue
                    )
                )
            }
            if (typedArray.hasValue(R.styleable.RubberRangePicker_initialEndValue)) {
                setCurrentEndValue(
                    typedArray.getInt(
                        R.styleable.RubberRangePicker_initialEndValue,
                        minValue
                    )
                )
            }
    ...
    override fun onLayout(changed: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
        ...
        if (startThumbX < trackStartX || !initialStartThumbXPositionQueue.isEmpty()) {
            if (initialStartThumbXPositionQueue.isEmpty()) {
                ...
                initialStartThumbXPositionQueue.poll()?.let { setCurrentStartValue(it) }
            }
            startThumbY = trackY
        }
        if (endThumbX < trackStartX || !initialEndThumbXPositionQueue.isEmpty()) {
            if (initialEndThumbXPositionQueue.isEmpty()) {
                ...
                initialEndThumbXPositionQueue.poll()?.let { setCurrentEndValue(it) }
            }
            endThumbY = trackY
        }
    }
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                if (thumbSelectedReference?.get() == true && thumbXReference != null && thumbYReference != null) {
                    ...
                    // Check if the touch event should trigger a click
                    if (isClick(x, y)) {
                        performClick()
                    }
                    return true
    ...
    private fun isClick(x: Float, y: Float): Boolean {
        // Define the threshold for considering a touch as a click
        val clickThreshold = ViewConfiguration.get(context).scaledTouchSlop
        return (x < clickThreshold && y < clickThreshold)
    }
    ...
    private fun getCurrentStartValue(): Int {
    ...
    private fun getCurrentEndValue(): Int {
    ...
    private fun setCurrentStartValue(value: Int) {
    ...
    private fun setCurrentEndValue(value: Int) {
        val validValue = value.coerceAtLeast(minValue).coerceAtMost(maxValue)
        if (trackEndX < 0) {
            //If this function gets called before the view gets layed out and learns what it's width value is
