...
import com.doubleclick.rovleapp.databinding.FragmentPaymentBinding
import com.doubleclick.rovleapp.dialog.DialogPaymentSuccess
...
class PaymentFragment : BaseFragment(R.layout.fragment_payment) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(viewModel) {
            observe(payOrder, ::handlePayOrder)
...
// Create and show the custom toast
                ...
                viewModel.doPayOrder(
                    convertedCreditCard.toString(),
                    convertedExpiryDate,
                    cardCvcString,
                    navArgs.orderRequest.provider_id,
                    navArgs.orderRequest.delivery_type,
                    navArgs.orderRequest.zip_code,
                    navArgs.orderRequest.name,
                    navArgs.orderRequest.phone,
                    navArgs.orderRequest.email,
                    navArgs.orderRequest.address,
                    navArgs.orderRequest.coffee_shop_id,
                    navArgs.orderRequest.locker_location,
                    navArgs.orderRequest.note,
                    "visa"
                )
...
// Create and show the custom toast
    ...
    private fun handlePayOrder(data: PayOrderData) {
        ...
        dialog.findViewById<Button>(R.id.terminar)?.apply {
            text = getString(R.string.order_completed)
            setOnClickListener {
                lifecycleScope.launch {
                    dialog.dismiss()
                    findNavController().navigate(PaymentFragmentDirections.actionPaymentFragmentToFacturaAfterPaymentFragment(data.id.toString()))
                }
            }
        }
