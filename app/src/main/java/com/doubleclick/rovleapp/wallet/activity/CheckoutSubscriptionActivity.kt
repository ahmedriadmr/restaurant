...
class CheckoutSubscriptionActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        with(viewModel) {
            observe(paySubscriptionGooglePay, ::renderPaySubscriptionGooglePay)
            doPaySubscriptionGooglePay(handlePaySubscriptionGooglePayRequest())
        }
    ...
    private fun renderPaySubscriptionGooglePay(data: PaySubscriptionGooglePayData) {
//        startActivity(Intent(this, CheckoutSuccessActivity::class.java))
    ...
    private fun handlePaymentSuccess(paymentData: PaymentData) {
        ...
        viewModel.doPaySubscriptionGooglePay(handlePaySubscriptionGooglePayRequest())
        try {
    ...
    private fun handlePaySubscriptionGooglePayRequest(): PaySubscriptionGooglePayRequest {
        val subscriptionDetailsReturn: SubscriptionData? = intent.getParcelableExtra("subscriptionDetails")
        if (subscriptionDetailsReturn != null ) {
            // Map cart items to update passport_offer_id if it's null

            return PaySubscriptionGooglePayRequest(
                subscriptionDetailsReturn.plan!!.provider_id,
                "subscription",
                subscriptionDetailsReturn.id,
                PaymentDetails(subscriptionDetailsReturn.total?.toDouble()?: 0.0,subscriptionDetailsReturn.shipping?.toDouble()?: 0.0,subscriptionDetailsReturn.taxes?.toDouble()?: 0.0,"test1234",
                    "google_pay","success","succeeded")
